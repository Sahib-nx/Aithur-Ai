...
const Prompt = ({ selectedChatId, onChatUpdate }) => {
    ...
    useEffect(() => {
        ...
        // Always update when selectedChatId changes OR when there's no currentChatId
        if (chatId !== currentChatId || !currentChatId) {
            setCurrentChatId(chatId);
            ChatManager.setCurrentChatId(chatId);
            
            // Load chat messages - this will be empty array for new chats
            const chat = ChatManager.getChat(chatId);
            setPrompt(chat?.messages || []);
            
            // Clear any typing state when switching chats
            setTypeMessage("");
            setIsTyping(false);
        }
